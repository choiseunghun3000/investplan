$(document).ready(function(){
    ...
    // news_section
    var news_section_arr = [ //[이미지, 타이틀, 내용, 작성일]
    ...
    for(i in news_section_arr){
        $("#news_section .wrap").append(`
        <div class="cont">
            <div class="img_part">
                <img src="./img/sub/${news_section_arr[i][0]}" alt="">
            </div>
            <div class="txt_part">
                <h3>${news_section_arr[i][1]}</h3>
                <p>${news_section_arr[i][2]}</p>
                <span>${news_section_arr[i][3]}</span>
            </div>
        </div>
        `);
    }
